...
function translate(s)
	local dictionary1 = string.split("a b c d e f g h i j k l m n o p q r s t u v w x y z", " ")
	local dictionary2 = string.split("ᔑ ʖ ᓵ ↸ ᒷ ⎓ ⊣ ⍑ ╎ ⋮ ꖌ ꖎ ᒲ リ 𝙹 !¡ ᑑ ∷ ᓭ ℸ ̣ ⚍ ⍊ ∴ ̇/ || ⨅" , " ")
	local sletters = string.split(string.lower(s), "")
	
	local news = ""
	for i, v in pairs(sletters) do
		local find = table.find(dictionary1, v)
		if find then
			news = news..dictionary2[find]
		else
			news = news..v
		end
	end
...
local json_string = [[
{
  "language.name": "English Enchanting Table",
